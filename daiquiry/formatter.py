...
class ExtrasFormatter(logging.Formatter):
    ...
    function to enable this functionality. Without it normal daiquiry
    ...
    def add_extras(self, record: types.ExtrasLogRecord) -> None:
        if not hasattr(record, "_daiquiry_extra_keys"):
            record.extras = ""
            return

        extras = self.extras_separator.join(
            self.extras_template.format(k, getattr(record, k))
            for k in record._daiquiry_extra_keys
            if k != "_daiquiry_extra_keys" and k not in self.keywords
        )
        if extras != "":
